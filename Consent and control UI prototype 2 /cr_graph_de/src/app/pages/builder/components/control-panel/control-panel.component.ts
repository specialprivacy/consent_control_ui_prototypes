...
export class ControlPanelComponent implements OnInit {
    ...
    onAcceptClick() {
        ...
        if (containsOptional && store.selectedOptionalPaths.length === 0) {
            this.dialog.open(InfoDialog2Component, {data: {text: 'Bitte wählen Sie mindestens ein optionales Element aus!'}});
        } else {
    ...
    onRevokeClick() {
        this.dialog.open(ConfirmDialogComponent, {data: {title: 'Achtung', text: 'Möchten Sie wirklich Ihre Zustimmung widerrufen? Die entsprechende Funktion wird nicht verfügbar sein.'}})
