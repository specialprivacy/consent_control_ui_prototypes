...
export class AdminComponent implements OnInit, OnDestroy {
    ...
    onUserClick(userd) {
        ...
        const qsn1 = this.questionnaireService.loadPreQuestionnaireByUserId(userd.uid).subscribe(x => {
            ...
            const qsn2 = this.questionnaireService.loadPostQuestionnaireByUserId(userd.uid).subscribe(y => {
                ...
                questionnaire2.push(new Questionnaire('usefulTab', 'Information on which tab or tabs did you find useful and informative?', { data: 'Data', sharing: 'Sharing', storage: 'Storage', purpose: 'Purpose', processing: 'Processing', none: 'None' }));
                questionnaire2.push(new Questionnaire('notUsefulTab', 'Which tab or tabs should be removed because they are useless and overcomplicate everything?', { data: 'Data', sharing: 'Sharing', storage: 'Storage', purpose: 'Purpose', processing: 'Processing', none: 'None' }));
                ...
                qstn += '\u000d\u000d';
                qstn += this.saveAnswers2(y, userd, questionnaire2, 'Questionnaire2');
    ...
    private saveAnswers(data: QuestionnaireItem[], user, questionnaires: Questionnaire[], header) {
        ...
        const qstIds = questionnaires.map(x => x.questionId);
        ...
        questionnaires.forEach(x => {
            ...
            try {
                ...
                } else if (x.questionId === 'wordsDescribe' || x.questionId === 'usefulTab' || x.questionId === 'notUsefulTab') {
                    tmp += '    - ';
    ...
    private saveAnswers2(data: QuestionnaireItem[], user, questionnaires: Questionnaire[], header) {
        if (!data) {
            data = [];
        }
        let result = header + '\u000d';
        ...
        const qstns = [
            'usefulTab', 'notUsefulTab', 'satisfiedConsentRequest', 'recommendConsentRequest', 'impressionTimeToComplete', 'wordsDescribe', 'concentMeetsNeeds',
            'understandableTreeGraph', 'usefulTreeGraph'
        ];
        questionnaires.forEach(x => {
            if (!questionnaireCodes.includes(x.questionId) || !qstns.includes(x.questionId)) {
                return;
            }
            try {
                const savedAnswer = data.find(item => item.questionCode === x.questionId);
    ...
    onShowClick() {
        this.userSubscription = this.userService.getAllUsers().subscribe(x => {
            ...
            const usersc = _.filter(x, user => {
                const ignore = _.split(this.toIgnore, ',').map(userId => crypto.SHA256(userId.trim()).toString() + '@userconcent-request.com');
